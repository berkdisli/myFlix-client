...
export class ProfileView extends React.Component {
  ...
  getUser(token) {
    ...
    axios.get(`https://berkdislimyflix.herokuapp.com/users/${username}`, {
  ...
  editUser(e) {
    ...
    axios.put(`https://berkdislimyflix.herokuapp.com/users/${username}`,
    {
  ...
  onRemoveFavorite(id) {
    ...
    axios.delete(`https://berkdislimyflix.herokuapp.com/users/${username}/favorites/` + (id), {
      headers: { Authorization: `Bearer ${token}` }
    })
    .then((response) => {
  ...
  onDeleteUser() {
    ...
    const token = localStorage.getItem('token');
    const username = localStorage.getItem('user');
    axios.delete(`https://berkdislimyflix.herokuapp.com/users/${username}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
    .then((response) => {
      console.log(response);
          ...
          <Col xs={12} sm={4}>
            <Card>
              <Card.Body>
                ...
                <Button className='delete-profile-button' variant="danger" onClick={() => this.onDeleteUser()} >Delete Profile</Button>
              </Card.Body>
            </Card>
          </Col>
            ...
            <Card>
              <Card.Body>
                ...
                <Form className="form-user" onSubmit={(e) => this.editUser(e)}>
                  ...
                  <Form.Group>
                    Email
                    <Form.Control type='email' name="Email" placeholder="New E-mail" onChange={(e) => this.setEmail(e.target.value)} required />
                  </Form.Group>
                  <Form.Group>
                    Birthday
